...
#include "Advection_Pressure.h"
#include "Function.h"

registerMooseObject("parrot_realApp", Advection_Pressure);

template <>
InputParameters
validParams<Advection_Pressure>()
{
...
RealVectorValue
Advection_Pressure::negSpeedQp() const
{
    RealVectorValue vel = -1.0 * _vel[_qp];
    
    return 1.0 * _k * _test[_i][_qp] * vel;
}

Real
Advection_Pressure::computeQpResidual()
{
...
Real
Advection_Pressure::computeQpJacobian()
{
    ...
    return negSpeedQp() * _grad_phi[_j][_qp];
